...
- [ ] 2.1 Write `SessionRegistry` Solidity contract: verifier registration with bond, `initSession(agent_pubkey, nonce, verifier_pubkey, challenge_hashes: bytes32[5])`, session spending, expiry, per-verifier rate limit (max 10 open sessions), `publishNonceRoot(epoch, merkle_root)`, `slashNonceReuse(operator_id, proof_a, proof_b)`
...
- [ ] 2.3 Write `SuccessionRegistry` Solidity contract: `commitSuccession(commitment)`, `revealSuccession(agent_id, old_pubkey, new_pubkey, salt)` with 24-hour timelock, `vetoSuccession(agent_id)` callable only by registered guardian, succession rate limiting (1h min, 100 max chain length), guardian rotation via same commit-reveal path
- [ ] 2.4 Add VRF operator sampling to the AVS coordinator contract (per-session random K-operator selection)
- [ ] 2.5 Write unit tests for `SessionRegistry`: duplicate nonce, unregistered verifier, rate limit, spending, expiry, challenge_hashes storage, slashNonceReuse valid proof, slashNonceReuse invalid proof, epoch monotonicity
- [ ] 2.6 Write unit tests for `AnchorIdentity`: group_pubkey + control_pubkey storage, both keys readable by operators, succession append-only, circular chain rejection
- [ ] 2.7 Write unit tests for `SuccessionRegistry`: commit records hash covering both key pairs, reveal before 24h reverts, reveal after 24h succeeds with both new keys, guardian veto cancels commitment, non-guardian veto reverts, rate limit (1h min, 100 max), guardian rotation
- [ ] 2.8 Deploy contracts to testnet; record addresses in config
...
- [ ] 4.1 Implement `requestSignature(message, sessionId)` — generate auth token `sign(session_id || message_hash, control_privkey)`, attach to request, coordinate VRF-sampled operator selection, broadcast, collect partial signatures
- [ ] 4.2 Implement operator auth token verification: `ed25519.verify(auth_token, session_id || message_hash, control_pubkey)` fetched from on-chain anchor; reject with receipt if absent or invalid; this check precedes all other operator checks
...
- [ ] 4.8 Write unit tests: successful 3-of-5 signing, missing auth token rejected, invalid auth token rejected, valid auth token proceeds, Round 1 pre-check rejects uncommitted challenge, Round 1 pre-check rejects malformed message, expiry + retry with new operator sample, non-sampled operator partial sig rejected, nonce scalars zeroed post-signing, final sig verification failure
...
- [ ] 6.4 Build identity record: `{ agent_id, threshold_pubkey, control_pubkey, eigenda_record_id: null, db_commitment: null, successor: null }`
...
- [ ] 7.4 Select 5 challenges from genesis corpus; compute `keccak256(challenge)` for each to form `challenge_hashes: bytes32[5]`
...
- [ ] 8.5 Add coordinator nonce commitment log: append `{ operator_id, session_id, D_i, E_i, z_i, epoch, timestamp }` per partial signature received; include rejection receipts
- [ ] 8.6 Implement `publishNonceRoot` call from coordinator after each round: compute Merkle root over new log entries, call on-chain
- [ ] 8.7 Write unit tests: resharing produces same public key, old shares rejected post-resharing, succession commit stored, reveal before 24h fails, reveal after 24h succeeds, guardian veto cancels commitment, chain traversal reaches current key, nonce log Merkle root published after round
